...
void ratioNeutrinoChiSquareGen(){
  ...
  Float_t AZH_ReconNeutrinoPZ = 0.;
  Float_t AZH_GeneratorNeutrinoPZ = 0.;

  tSelected->SetBranchAddress("AZH_ReconNeutrinoPZ", &AZH_ReconNeutrinoPZ);
  tSelected->SetBranchAddress("AZH_GeneratorNeutrinoPZ", &AZH_GeneratorNeutrinoPZ);


  TCanvas *c = new TCanvas("c", "canvas", 800, 800);

  TH1F* histo1 = new TH1F("AZH_NeutrinoPZ", "AZH_NeutrinoPZ", 40,-400, 400);
  TH1F* histo2 = new TH1F("NeutrinoPZ", "NeutrinoPZ", 40,-400, 400);
  TH1F* hratio = new TH1F("NeutrinoPZ-ratio", "NeutrinoPZ-ratio", 40,-400, 400);
  ...
  for (int i = 0; i < nEntries; i++) {
    ...
    if (i%1000) std::cout << "reading event: " << i << std::endl;
          if (AZH_ReconNeutrinoPZ != -9999) {
              histo1->Fill(AZH_ReconNeutrinoPZ);
  ...
  legend->AddEntry(histo1, "chisquared-matched ", "f");
  ...
  h3->GetYaxis()->SetTitle("ratio chisquared/true ");
  ...
  c->Update();
  c->Print("ratioNeutrinoGeneratorTrueAndChiSquaredMatched.png");
}
