...
class l3KinProducer(Module):
    ...
    newbranches = {
        ...
        'AZH_Amass':  (["F"], {}),
        'AZH_Hmass' : (["F"], {}),
        'AZH_ReconNeutrinoPZ': (["F"], {}),
        'AZH_GeneratorMatchedNeutrinoPZ': (["F"], {}),
        'AZH_GeneratorNeutrinoPZ': (["F"], {}),
    }
    ...
    def AZH_ReconNeutrinoPZ(self):
        if self.passSelection is True:
            return self.bestNeutrinoPZ #this it he neturino pz from the chisquared
        else:
            return -9999

    def AZH_GeneratorMatchedNeutrinoPZ(self):
        if self.passSelection is True:
            return self.chooseThisNeutrinoPZ #this is the neutrino pz from choosing the solution closest to the true generator pz
        else:
            return -9999

    def AZH_GeneratorNeutrinoPZ(self):
        if self.passSelection is True:
            return self.trueNeutrinoPZ #this is the true neutrino pz from the generator
        else:
            return -9999
...
##code added below - jieun****************************************************
        ...
        self.bestChiSquare = 9999
        self.bestLeptonic = 9999
        self.bestHadronic = 9999
        ...
        if self.passSelection:
            ...
            #take the real part of solution if complexx
            if sol > 0:
                sol = math.sqrt(sol)
            else:
                sol = 0
            #these are two possible pzs for neutrino; for generator level,find the neutrino pz closest to the real one
            ...
            #END NEUTRINO section **********************************************************************************************************

            ## add this section temporarily - since there was a request to see how good the reconstruction of the neutrino pz was.
            #self.neutrionpz1 and self.neutrinopz2 are the two possible solutions. the chi-squared uses both as input, and chooses the 'best one'
            #let's make two graphs - 1. generator pz vs pz using Prof. Mills' formula; and 2. generator pz vs. chisquared ones
            #save the chisquared chosen pz as a float as self.bestNeutrinoPZ
            #self.neutrino1, self.neutrino2 are TLorentzVectors with the two possible neutrinos
            #generator level (this will be erased in the final version that just has reconstruction-level variables)
            #***********************************************************************************
            #will delete this below part later since not needed for recon.- this gets the real neutrino PZ - use for valid. studies
            genPartList = Collection(event, "GenPart")
            self.genPart_list = []
            self.countquarks = 0
            self.countleptons = 0
            self.IsAZH = False
            self.trueNeutrinoPZ = 0
            ...
            if self.IsAZH is True:
                ...
                self.trueNeutrinoPZ = self.neutrino.Pz()
                print('true neutrino pz', self.trueNeutrinoPZ)

            self.generatorMatchedNeutrino = ROOT.TLorentzVector()
            ...
            self.GeneratorMatchedNeutrinoEnergy = math.sqrt( pow(self.MET.Pt(),2) + pow(self.chooseThisNeutrinoPZ,2) )
            self.generatorMatchedNeutrino.SetPxPyPzE(self.MET.Px(),self.MET.Py(),self.chooseThisNeutrinoPZ,self.GeneratorMatchedNeutrinoEnergy)

            print('true, guess1, guess2', self.trueNeutrinoPZ, self.neutrinopz1, self.neutrinopz2, self.chooseThisNeutrinoPZ)
            #*************************************************************************
            ...
            # this is a list of neutrinos and b-jets
            self.tryThisList= [ self.neutrinoList, self.bjets]
            #this is a list of all possibly combinatiosn of neutrinos and b-jets
            ...
            if self.bestChiSquareNu1 <= self.bestChiSquareNu2:
                ...
                self.bestNeutrinoPZ = self.neutrinoTLorentz.Pz() #this is used to test our good our neutrino pz recon. is
            else:
                ...
                self.bestNeutrinoPZ = self.neutrinoTLorentz2.Pz()
            print('bestneutrinoPz', self.bestNeutrinoPZ)
